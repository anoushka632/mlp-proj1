...
#%%
rf_c = RandomForestClassifier(random_state = 786)

scores = cross_val_score(rf_c, X_train, y_train.flatten(), scoring='accuracy',
                         cv= 4, n_jobs=-1)

rf_scores = np.mean(np.absolute(scores))
rf_scores

rf_c.fit(X_test, y_test)
...
y_pred = rf_c.predict_proba(X_test)# predicted probabilities generated by sklearn classifier
...
# %%
y_true = y_test# ground truth labels
y_pred = fin_xgb.predict_proba(X_test)# predicted probabilities generated by sklearn classifier
skplt.metrics.plot_roc_curve(y_true, y_pred)
plt.show()
# %%
